...
##Import Data across Indonesia location comparisons
...
for loc in locations:
    print(loc)
    #2x pulse for GF
    dict_conc['Indo_Jan_' + loc] = xr.open_mfdataset(f'{utils.geos_chem_data_path}stretch_{pulse_size}_pulse/loc_uncertainty_Indo/{loc}_pulse/mod_output/GEOSChem.SpeciesConc.{time}*', combine = 'by_coords')
    #2x pulse for GF
    dict_emis['Indo_Jan_' + loc] = xr.open_mfdataset(f'{utils.geos_chem_data_path}stretch_{pulse_size}_pulse/loc_uncertainty_Indo/{loc}_pulse/mod_output/GEOSChem.Emissions.{time}*', combine = 'by_coords')
...
xr.concat([full_ds[r] for r in full_ds.keys()], pd.Index([r for r in full_ds.keys()], name='run')).to_netcdf(f'{utils.data_output_path}/greens_functions/Greens_function_{poll_name}_loc_Indonesia.nc4', mode = 'w')
