...
for d in days:
    print(d)
    #2x pulse for GF
    dict_conc['Indo_Jan_' + d] = xr.open_mfdataset(f'{utils.geos_chem_data_path}stretch_{pulse_size}_pulse/Jan_uncertainty_Indo/Jan{d}_pulse/mod_output/GEOSChem.SpeciesConc.{time}*', combine = 'by_coords')
    #2x pulse for GF
    dict_emis['Indo_Jan_' + d] = xr.open_mfdataset(f'{utils.geos_chem_data_path}stretch_{pulse_size}_pulse/Jan_uncertainty_Indo/Jan{d}_pulse/mod_output/GEOSChem.Emissions.{time}*', combine = 'by_coords')
...
##Import base data        
for d in days:
    dict_conc[f'base_Jan_{d}'] = xr.open_mfdataset(f'{utils.geos_chem_data_path}stretch_base/template/Jan{d}_pulse/mod_output/GEOSChem.SpeciesConc.{time}*', combine = 'by_coords', engine = 'netcdf4')
    dict_emis[f'base_Jan_{d}'] = xr.open_mfdataset(f'{utils.geos_chem_data_path}stretch_base/template/Jan{d}_pulse/mod_output/GEOSChem.Emissions.{time}*', combine = 'by_coords', engine = 'netcdf4')
...
print(full_ds[r].max().values)


### Save out the Green's function
xr.concat([full_ds[r] for r in full_ds.keys()], pd.Index([r for r in full_ds.keys()], name='run')).to_netcdf(f'{utils.data_output_path}/greens_functions/Greens_function_{poll_name}_Jan_dates_Indonesia.nc4', mode = 'w')
